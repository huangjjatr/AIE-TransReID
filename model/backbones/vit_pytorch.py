...
class LeakyTriangle(Function):

    @staticmethod
    def forward(ctx, x, eps=5e-4):
        abs_x = torch.abs(x)
        result = 1.0 - abs_x
        index = result < eps
        if result.dtype == torch.float16:
            result[index] = (eps * torch.exp(result[index] - eps)).half()
            # result[index] = (eps * (1 - eps) / abs_x[index]).half()
        else:
            result[index] = eps * torch.exp(result[index] - eps)
            # result[index] = (eps * (1 - eps) / abs_x[index])
    ...
    def backward(ctx, grad_output):
        ...
        grad = -torch.sign(x)
        index = result < ctx.epsilon
        grad[index] *= result[index]
        # grad[index] = -result[index] / x[index]
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 sie_xishu1 = 0.0, text_attr = 'market'):
        super().__init__()
        ...
        if sie_xishu < 0:
            self.sie_xishu = nn.Parameter(torch.tensor(-sie_xishu))
        else:
            self.sie_xishu = sie_xishu
        
        self.sie_xishu1 = sie_xishu1  # original SIE coefficient
        ...
        self.text_embed = nn.Linear(512, embed_dim)
        if torch.__version__ < '2.':
            text_features =  torch.load(f'text/text_feature_{text_attr}.pt') + [torch.load('text/background.pt')]
        else:
            text_features =  torch.load(f'text/text_feature_{text_attr}.pt', weights_only=True) + [torch.load('text/background.pt', weights_only=True)]
        
        self.text_features = torch.stack(text_features, dim=0).to("cuda")
        print(f'using object attribute text features {self.text_features.shape} generated by CLIP')
    ...
    def forward_features(self, x, label, camera_id, view_id):
        B = x.shape[0]
        ...
        if self.cam_num > 0 and self.view_num > 0:
            x = x + self.pos_embed + self.sie_xishu1 * self.sie_embed[camera_id * self.view_num + view_id]
        elif self.cam_num > 0:
            x = x + self.pos_embed + self.sie_xishu1 * self.sie_embed[camera_id]
        elif self.view_num > 0:
            x = x + self.pos_embed + self.sie_xishu1 * self.sie_embed[view_id]
        else:
            x = x + self.pos_embed
        
        x = x + self.sie_xishu * self.text_embed(self.text_features[label])
        x = self.pos_drop(x)
